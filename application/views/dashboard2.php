      ...
      <div class="layout-content">
        ...
        <?php 
          ...
          $flashdata = $this->session->flashdata('someone');
        ...
        <?php } ?> 
          <div class="row">
            <div class="col-sm-12">
              <div class="demo-form-wrapper">
                  <?php echo form_open_multipart('excelhandler/import_daily_roaster', 'class="form form-horizontal"');?>  
                  <div class="form-group">
                    <label class="col-sm-3 control-label" for="form-control-9">Person Roster</label>
                    <div class="col-sm-4">
                      <input id="form-control" name="userfile" id="userfile1" class="btn btn-default" type="file" required>
                    </div>
                    <div class="col-sm-1">
                      <a href='<?=base_url();?>assets/daily_roaster.xlsx' target="_blank"><span class="icon icon-file-excel-o"></span></a>
                    </div>
                    <div class="col-sm-1">
                      ...
                      <input data-toggle="modal" data-target="#modalBounceIn" type="hidden" id="popUp">
                    ...
                    <?php
                      ...
                      $person_roster = $this->db->query("SELECT * FROM person_daily_roaster where login_id = '".$session_data['id']."'")->result();
          
                      if(count($person_roster) > 0){

                    ?>
                    <!-- <div class="col-sm-2"><button onclick="deleteroster()">sfjdsaj</button></div>  -->
                    <?php  } ?>
                  </div>
                  </form>
                </div>
          ...
          <?php 
              ...
              $person_daily_roaster = $this->db->query("SELECT DISTINCT pdr.person_id,pdr.person_address,drll.latitude,drll.longitude,drll.create_time FROM person_daily_roaster pdr LEFT JOIN daily_roaster_lat_lng drll ON pdr.person_id=drll.person_id where pdr.login_id = '".$session_data['id']."'")->result();


              if(count($person_daily_roaster) > 0){
          ?>      
                ...
                <div class="card-body">
                  <table id="demo-datatables-1" class="table table-striped table-nowrap dataTable" cellspacing="0" width="100%">
                    ...
                    <tbody>
                      ...
                      $value = $this->db->query("SELECT pdr.person_id,pdr.person_address,drll.latitude,drll.longitude,drll.create_time FROM person_daily_roaster pdr LEFT JOIN daily_roaster_lat_lng drll ON pdr.person_id=drll.person_id where pdr.login_id = '".$session_data['id']."'")->result();
                      ...
                      <tr>
                        ...
                        <td><?=$value[$pr]->person_id; ?></td>
                        <td><?=$value[$pr]->person_address; ?></td>
                        <td><?=$value[$pr]->latitude; ?></td>
                        <td><?=$value[$pr]->longitude; ?></td>
                        <td><?=$value[$pr]->create_time; ?></td>
    ...
    <div id="modalBounceIn" tabindex="-1" role="dialog" class="modal in">
      <div class="modal-dialog">
        <div class="modal-content animated bounceIn">
                      ...
                      <table id="demo-datatables-1" class="table table-striped table-nowrap dataTable" cellspacing="0" width="100%">
                        ...
                        <form id="additionform" autocomplete="off">
                          <?php 
                            ...
                            for($pr=0;$pr < count($flashdata);$pr++) { 

                          ?>
                          <tr>
                            
                            <td><?=$flashdata[$pr]['person_id']; ?></td>
                            <input class="form-control" type="hidden" name="flashdataIdAddress" value="<?=$flashdata[$pr]['person_id']."##".$flashdata[$pr]['person_address']; ?>">
                            <td><?=$flashdata[$pr]['person_address']; ?></td>
                            </tr>
                          <?php } ?>
                        </form>  
                        </tbody>
          ...
          <div class="m-t-lg">
            <button class="btn btn-success" data-dismiss="modal" type="button" onclick="add_person_roster()">Continue</button>
    ...
    <script type="text/javascript">
      ...
      function add_person_roster()
      {
          ...
          var dataString = $('#additionform').serializeArray();
          $.ajax({
            url : "<?php echo site_url('excelhandler/flashdata_save')?>",
            type: "POST",
            data: {dataValue: dataString },
      ...
      function deleteroster()
      {
        if(confirm('Are you sure delete record?'))
        {
          // ajax delete data from database
            $.ajax({
              url : "<?php echo site_url('excelhandler/delete_daily_roaster')?>/",
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
